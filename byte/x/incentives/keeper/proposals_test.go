		...
		},
		{
			...
			func() {
				...
				err := suite.app.BankKeeper.MintCoins(
					suite.ctx,
					types.ModuleName,
					sdk.Coins{sdk.NewInt64Coin(denobyte, 1)},
				)
		...
		},
		{
			...
			func() {
				...
				_, err = suite.app.IncentivesKeeper.RegisterIncentive(
					...
					contract2,
					sdk.DecCoins{
						sdk.NewDecCoinFromDec(denobyte, sdk.NewDecWithPrec(100, 2)),
					},
					epochs,
				)
				suite.Require().NoError(err)
			...
			[]sdk.DecCoin{sdk.NewDecCoinFromDec(denobyte, sdk.NewDecWithPrec(100, 2))},
			false,
		},
		{
			"ok",
