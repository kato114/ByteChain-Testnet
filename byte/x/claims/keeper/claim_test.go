...
func (suite *KeeperTestSuite) TestClaibytesForAction() {
	addr := sdk.AccAddress(tests.GenerateAddress().Bytes())
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			amt, err := suite.app.ClaimsKeeper.ClaibytesForAction(suite.ctx, addr, tc.claimsRecord, tc.action, tc.params)
			if tc.expError {
...
func (suite *KeeperTestSuite) TestHookOfUnclaimableAccount() {
	...
	_, err := suite.app.ClaimsKeeper.ClaibytesForAction(suite.ctx, addr1, claim, types.ActionEVM, params)
	suite.Require().NoError(err)
...
func (suite *KeeperTestSuite) TestHookBeforeAirdropStart() {
	...
	claimedAmount, err := suite.app.ClaimsKeeper.ClaibytesForAction(suite.ctx, addr1, claimsRecord, types.ActionVote, params)
	suite.Require().NoError(err)
	...
	_, err = suite.app.ClaimsKeeper.ClaibytesForAction(suite.ctx.WithBlockTime(airdropStartTime), addr1, claimsRecord, types.ActionVote, params)
	suite.Require().NoError(err)
...
func (suite *KeeperTestSuite) TestHookAfterAirdropEnd() {
	...
	_, err = suite.app.ClaimsKeeper.ClaibytesForAction(suite.ctx, addr1, claimsRecord, types.ActionDelegate, params)
	suite.Require().NoError(err)
}
...
func (suite *KeeperTestSuite) TestDuplicatedActionNotWithdrawRepeatedly() {
	...
	_, err := suite.app.ClaimsKeeper.ClaibytesForAction(suite.ctx, addr1, claimsRecord, types.ActionEVM, params)
	suite.Require().NoError(err)
...
func (suite *KeeperTestSuite) TestAirdropFlow() {
	...
	// do half of actions
	_, err := suite.app.ClaimsKeeper.ClaibytesForAction(suite.ctx, addrs[0], claimsRecord, types.ActionEVM, params)
	suite.Require().NoError(err)
	_, err = suite.app.ClaimsKeeper.ClaibytesForAction(suite.ctx, addrs[0], claimsRecord, types.ActionDelegate, params)
	suite.Require().NoError(err)
	...
	// do rest of actions
	_, err = suite.app.ClaimsKeeper.ClaibytesForAction(suite.ctx, addrs[0], claimsRecord, types.ActionIBCTransfer, params)
	suite.Require().NoError(err)
	_, err = suite.app.ClaimsKeeper.ClaibytesForAction(suite.ctx, addrs[0], claimsRecord, types.ActionVote, params)
	suite.Require().NoError(err)
	...
	t := []struct {
		...
		},
		{
			fn: func() {
				...
				_, err := suite.app.ClaimsKeeper.ClaibytesForAction(ctx, addr1, claimsRecord, types.ActionEVM, params)
				suite.Require().NoError(err)
...
func (suite *KeeperTestSuite) TestClawbackEscrowedTokens() {
	...
	claimedCoins, err := suite.app.ClaimsKeeper.ClaibytesForAction(ctx, addr1, claimsRecord, types.ActionEVM, params)
	suite.Require().NoError(err)
	coins = suite.app.ClaimsKeeper.GetModuleAccountBalances(ctx)
	...
	claimedCoinsAfter, err := suite.app.ClaimsKeeper.ClaibytesForAction(ctx, addr1, claimsRecord, types.ActionDelegate, params)
	suite.Require().Error(err)
