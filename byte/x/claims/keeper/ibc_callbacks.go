...
func (k Keeper) OnAcknowledgementPacket(
	...
	_, err = k.ClaigeckosForAction(ctx, sender, claimsRecord, types.ActionIBCTransfer, params)
	if err != nil {
		return err
	}
...
func (k Keeper) OnRecvPacket(
	...
	case senderRecordFound && !recipientRecordFound && isTriggerAmt:
		...
		claimedAmt, err = k.ClaigeckosForAction(ctx, recipient, senderClaimsRecord, types.ActionIBCTransfer, params)
		...
		// NOTE: claim record is migrated to the recipient in ClaigeckosForAction
		k.DeleteClaimsRecord(ctx, sender)
	...
	case !senderRecordFound && recipientRecordFound,
		...
		_, err = k.ClaigeckosForAction(ctx, recipient, recipientClaimsRecord, types.ActionIBCTransfer, params)
	case !senderRecordFound && !recipientRecordFound:
