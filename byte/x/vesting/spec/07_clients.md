...
Find below a list of `byted` commands added with the `x/vesting` module. You can obtain the full list by using the `byted -h` command.
...
```go
byted add-genesis-account ADDRESS_OR_KEY_NAME COIN... [flags]
```
...
```go
byted query vesting balances ADDRESS [flags]
```
...
```go
byted tx vesting create-clawback-vesting-account TO_ADDRESS [flags]
```
...
```go
byted tx vesting clawback ADDRESS [flags]
```
...
```go
byted tx vesting update-vesting-funder VESTING_ACCOUNT_ADDRESS NEW_FUNDER_ADDRESS --from=FUNDER_ADDRESS [flags]
```
