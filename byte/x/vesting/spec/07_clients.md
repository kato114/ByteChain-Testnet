...
Find below a list of `geckod` commands added with the `x/vesting` module. You can obtain the full list by using the `geckod -h` command.
...
```go
geckod add-genesis-account ADDRESS_OR_KEY_NAME COIN... [flags]
```
...
```go
geckod query vesting balances ADDRESS [flags]
```
...
```go
geckod tx vesting create-clawback-vesting-account TO_ADDRESS [flags]
```
...
```go
geckod tx vesting clawback ADDRESS [flags]
```
...
```go
geckod tx vesting update-vesting-funder VESTING_ACCOUNT_ADDRESS NEW_FUNDER_ADDRESS --from=FUNDER_ADDRESS [flags]
```
