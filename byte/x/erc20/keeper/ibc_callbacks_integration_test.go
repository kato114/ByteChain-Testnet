...
var _ = Describe("Convert receiving IBC to Erc20", Ordered, func() {
	...
	Describe("enabled params and registered uosmo", func() {
		...
		It("should transfer and not convert byte", func() {
			// Register 'byte' coin in ERC-20 keeper to validate it is not converting the coins when receiving 'byte' thru IBC
			...
			// 1. Send byte from Evmos to Osmosis
			...
			// check ibc byte coins balance on Osmosis
			...
			// 2. Send byte IBC coins from Osmosis to Evmos
			...
			// check ibc byte coins balance on Osmosis - should be zero
			...
			// check byte balance after transfer - should be equal to initial balance
	...
	Describe("Performing claims with registered coin", func() {
		...
		It("it should perform the claim and convert the received tokens", func() {
			...
			// should trigger claims logic and send byte coins from claims to receiver
