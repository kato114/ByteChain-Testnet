...
Find below a list of  `byted` commands added with the  `x/erc20` module. You can obtain the full list by using the `byted -h` command. A CLI command can look like this:

```bash
byted query erc20 params
```
...
```bash
byted tx gov submit-proposal register-coin METADATA_FILE [flags]
```
...
```bash
byted tx gov submit-proposal register-erc20 ERC20_ADDRESS... [flags]
```
...
```bash
byted tx gov submit-proposal toggle-token-conversion TOKEN [flags]
```
