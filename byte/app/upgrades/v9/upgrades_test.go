...
func (suite *UpgradeTestSuite) TestReturnFundsFromCommunityPool() {
	...
	coins := sdk.NewCoins(sdk.NewCoin("byte", res))
	...
	suite.Require().Equal(balanceBefore.AmountOf("byte"), sdk.NewDecFromInt(res))
	...
	for i := range v9.Accounts {
		...
		balance := suite.app.BankKeeper.GetBalance(suite.ctx, addr, "byte")
