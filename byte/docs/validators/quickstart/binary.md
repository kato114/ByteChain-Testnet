...
`byted` is the all-in-one command-line interface. It supports wallet management, queries and transaction operations {synopsis}
...
After you have obtained the latest `byted` binary, run:
...
::: tip
You can also enable auto-completion with the `byted completion` command. For example, at the start of a bash session, run `. <(byted completion)`, and all `byted` subcommands will be auto-completed.
:::

### Config and data directory

By default, your config and data are stored in the folder located at the `~/.byted` directory.
...
To specify the `byted` config and data storage directory; you can update it using the global flag `--home <directory>`

### Configuring the Node

The Cosmos SDK automatically generates two configuration files inside `~/.byted/config`:
...
To set pruning, adjust the `pruning` parameter in the `~/.byted/config/app.toml` file.
...
We can view the default client config setting by using `byted config` command:
...
A list of commonly used flags of `byted` is listed below:

| Option              | Description                   | Type                                             | Default Value |
| ------------------- | ----------------------------- | ------------------------------------------------ | ------------- |
| `--chain-id`        | Full Chain ID                 | `string`                                         | `""`          |
| `--home`            | Directory for config and data | `string`                                         | `~/.byted`   |
| `--keyring-backend` | Select keyring's backend      | `{"os"|"file"|"kwallet"|"pass"|"test"|"memory"}` | `"os"`        |
| `--output`          | Output format                 | `string`                                         | "text"        |

## Command list

A list of commonly used `byted` commands. You can obtain the full list by using the `byted -h` command.
