...
`geckod` is the all-in-one command-line interface. It supports wallet management, queries and transaction operations {synopsis}
...
After you have obtained the latest `geckod` binary, run:
...
::: tip
You can also enable auto-completion with the `geckod completion` command. For example, at the start of a bash session, run `. <(geckod completion)`, and all `geckod` subcommands will be auto-completed.
:::

### Config and data directory

By default, your config and data are stored in the folder located at the `~/.geckod` directory.
...
To specify the `geckod` config and data storage directory; you can update it using the global flag `--home <directory>`

### Configuring the Node

The Cosmos SDK automatically generates two configuration files inside `~/.geckod/config`:
...
To set pruning, adjust the `pruning` parameter in the `~/.geckod/config/app.toml` file.
...
We can view the default client config setting by using `geckod config` command:
...
A list of commonly used flags of `geckod` is listed below:

| Option              | Description                   | Type                                             | Default Value |
| ------------------- | ----------------------------- | ------------------------------------------------ | ------------- |
| `--chain-id`        | Full Chain ID                 | `string`                                         | `""`          |
| `--home`            | Directory for config and data | `string`                                         | `~/.geckod`   |
| `--keyring-backend` | Select keyring's backend      | `{"os"|"file"|"kwallet"|"pass"|"test"|"memory"}` | `"os"`        |
| `--output`          | Output format                 | `string`                                         | "text"        |

## Command list

A list of commonly used `geckod` commands. You can obtain the full list by using the `geckod -h` command.
