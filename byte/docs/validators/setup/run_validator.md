...
```bash
byted tendermint show-validator
```
...
```bash
byted tx staking edit-validator
  --moniker="choose a moniker" \
...
```bash
byted query staking validator <account_cosmos>
```
...
```bash
byted query slashing signing-info <validator-pubkey>\
  --chain-id=<chain_id>
```
...
```bash
byted query tendermint-validator-set | grep "$(byted tendermint show-address)"
```

You should now see your validator in one of Evmos explorers. You are looking for the `bech32` encoded `address` in the `~/.byted/config/priv_validator.json` file.
...
you want your node to shutdown or by passing the `--halt-height` flag to `byted`.
...
If you got jailed for downtime, you can get your voting power back to your validator. First, if `byted` is not running, start it up again:
...
The default number of files Linux can open (per-process) is `1024`. `byted` is known to open more than `1024` files. This causes the process to crash. A quick fix is to run `ulimit -n 4096` (increase the number of open files allowed) and then restart the process with `byted start`. If you are using `systemd` or another process manager to launch `byted` this may require some configuration at that level. A sample `systemd` file to fix this issue is below:

```toml
# /etc/systemd/system/byted.service
[Unit]
...
ExecStart=/home/ubuntu/go/bin/byted start
Restart=on-failure
RestartSec=3
LimitNOFILE=4096
