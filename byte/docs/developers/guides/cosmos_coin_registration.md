...
To [submit the proposal](../../users/governance/submitting.md) to testnet through the command line with [`byted`](../../validators/quickstart/binary.md), use the following command with `register-coin`:

```bash
byted tx gov submit-proposal register-coin <path/to/metadata.json> \
  --title=<title> \
...
```bash
byted tx gov submit-proposal register-coin --proposal=<path/to/proposal.json>
```
