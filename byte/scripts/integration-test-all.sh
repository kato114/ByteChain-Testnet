...
## default port prefixes for geckod
NODE_P2P_PORT="2660"
NODE_PORT="2663"
NODE_RPC_PORT="2666"
...
init_func() {
    "$PWD"/build/geckod keys add $KEY"$i" --keyring-backend test --home "$DATA_DIR$i" --no-backup --algo "eth_secp256k1"
    "$PWD"/build/geckod init $MONIKER --chain-id $CHAINID --home "$DATA_DIR$i"
    "$PWD"/build/geckod add-genesis-account \
    "$("$PWD"/build/geckod keys show "$KEY$i" --keyring-backend test -a --home "$DATA_DIR$i")" 1000000000000000000pose,1000000000000000000stake \
    --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/geckod gentx "$KEY$i" 1000000000000000000stake --chain-id $CHAINID --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/geckod collect-gentxs --home "$DATA_DIR$i"
    "$PWD"/build/geckod validate-genesis --home "$DATA_DIR$i"
...
start_func() {
    ...
    "$PWD"/build/geckod start --pruning=nothing --rpc.unsafe \
